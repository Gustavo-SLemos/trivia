...
class Pergunta {
    ...
    public function gerarPerguntaAPI() {
        ...
        foreach ($data["results"] as $questao) {
            ...
            foreach ($opcoes as $opcao) {
                echo "<input class='opcao' type='radio' name='id_pergunta' value=$opcao>$opcao<br>";
            }
        }
    ...
    public function gerarPerguntaBD(){
        ...
        echo "<h2 class='pergunta'>" . $pergunta["question"] . "</h2>" . "<br>";
        
        $opcoes = array_merge([$pergunta['correct_answer']], json_decode($pergunta['incorrect_answers']));//decode para o json de incorretas
        shuffle($opcoes);
